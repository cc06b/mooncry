#include <stdio.h>
#include <stdint.h>
#include <string.h>

uint32_t rotr(uint32_t x, int n) { return (x >> n) | (x << (32-n)); }

uint32_t K[64] = {
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
    0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
    0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
    0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
};

void sha256_compress(uint32_t state[8], uint32_t block[16]) {
    uint32_t msg[64];
    for (int i = 0; i < 16; i++) msg[i] = block[i];
    for (int i = 16; i < 64; i++) {
        uint32_t s1 = rotr(msg[i-2], 17) ^ rotr(msg[i-2], 19) ^ (msg[i-2] >> 10);
        uint32_t s0 = rotr(msg[i-15], 7) ^ rotr(msg[i-15], 18) ^ (msg[i-15] >> 3);
        msg[i] = msg[i-16] + msg[i-7] + s0 + s1;
    }
    uint32_t a = state[0], b = state[1], c = state[2], d = state[3];
    uint32_t e = state[4], f = state[5], g = state[6], h = state[7];
    for (int i = 0; i < 64; i++) {
        uint32_t S1 = rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25);
        uint32_t ch = (e & f) ^ ((~e) & g);
        uint32_t t1 = h + S1 + ch + K[i] + msg[i];
        uint32_t S0 = rotr(a, 2) ^ rotr(a, 13) ^ rotr(a, 22);
        uint32_t maj = (a & b) ^ ((a & c) ^ (b & c));
        uint32_t t2 = S0 + maj;
        h = g; g = f; f = e;
        e = d + t1;
        d = c; c = b; b = a;
        a = t1 + t2;
    }
    state[0] += a; state[1] += b; state[2] += c; state[3] += d;
    state[4] += e; state[5] += f; state[6] += g; state[7] += h;
}

void sha256_hash(const char* data, int len) {
    uint32_t state[8] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
                          0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};

    int full_bytes = (len / 64) * 64;
    int pos = 0;

    // Process full 64-byte blocks
    while (pos < full_bytes) {
        uint32_t block[16];
        for (int i = 0; i < 16; i++) {
            int off = pos + i*4;
            block[i] = ((uint8_t)data[off] << 24) | ((uint8_t)data[off+1] << 16) |
                       ((uint8_t)data[off+2] << 8) | (uint8_t)data[off+3];
        }
        sha256_compress(state, block);
        pos += 64;
    }

    // Last block with padding
    uint32_t block[16] = {0};
    int rem = len - full_bytes;
    int block_idx = 0;
    int byte_pos = full_bytes;

    while (byte_pos < len && block_idx < 14) {
        uint32_t word_val = 0;
        for (int j = 0; j < 4; j++) {
            if (byte_pos < len)
                word_val |= (uint8_t)data[byte_pos] << (24 - j*8);
            byte_pos++;
        }
        block[block_idx++] = word_val;
    }

    // Add 0x80 padding
    if (block_idx < 14) {
        int pos_in_word = len % 4;
        if (pos_in_word == 0) {
            block[block_idx++] = 0x80 << 24;
        } else {
            block[block_idx++] |= 0x80 << (24 - pos_in_word * 8);
        }
    }

    // Add length
    if (block_idx < 15) {
        uint64_t bits = (uint64_t)len * 8;
        block[14] = bits >> 32;
        block[15] = bits & 0xFFFFFFFF;
        sha256_compress(state, block);
    } else {
        block[14] = 0; block[15] = 0;
        sha256_compress(state, block);
        block[14] = ((uint64_t)len * 8) >> 32;
        block[15] = ((uint64_t)len * 8) & 0xFFFFFFFF;
        sha256_compress(state, block);
    }

    printf("SHA-256(\"%s\") = ", data);
    for (int i = 0; i < 8; i++) {
        printf("%08x", state[i]);
    }
    printf("\n");
}

int main() {
    sha256_hash("a", 1);
    sha256_hash("abc", 3);
    sha256_hash("", 0);

    // Test 56 bytes
    char data56[56];
    memset(data56, 'a', 56);
    sha256_hash(data56, 56);

    return 0;
}
