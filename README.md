...
- **Correct** — every algorithm is checked against FIPS / NIST / RFC test
  ...
  differential testing. 1150 tests, run with `moon test --deny-warn`.
...
**ZUC** (`lib/zuc_test.mbt`, 8 tests) is vector-verified twice over: the
...
binary before being stored. On top of the vectors: stream round-trips at 18
lengths, keystream determinism and prefix consistency, one-bit key/IV
separation, and the mode semantics that the vectors alone do not pin — bits
beyond LENGTH must not influence EEA3's output or EIA3's MAC, a flipped in-range
bit must break the MAC, and COUNT/BEARER/DIRECTION/key must each be bound into
it.
