#!/usr/bin/env python3
import struct

# S-Box
S_BOX = [
    
]

def gmul(a, b):
    p = 0
    for _ in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80
        a = (a << 1) & 0xFF
        if hi_bit_set:
            a ^= 0x1b
        b >>= 1
    return p

def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = S_BOX[state[i][j]]

def shift_rows(state):
    # Row 1: shift left by 1
    state[1] = state[1][1:] + state[1][:1]
    # Row 2: shift left by 2
    state[2] = state[2][2:] + state[2][:2]
    # Row 3: shift left by 3
    state[3] = state[3][3:] + state[3][:3]

def mix_columns(state):
    for i in range(4):
        s0, s1, s2, s3 = state[0][i], state[1][i], state[2][i], state[3][i]
        state[0][i] = gmul(0x02, s0) ^ gmul(0x03, s1) ^ s2 ^ s3
        state[1][i] = s0 ^ gmul(0x02, s1) ^ gmul(0x03, s2) ^ s3
        state[2][i] = s0 ^ s1 ^ gmul(0x02, s2) ^ gmul(0x03, s3)
        state[3][i] = gmul(0x03, s0) ^ s1 ^ s2 ^ gmul(0x02, s3)

def bytes_to_state(data):
    state = [[0]*4 for _ in range(4)]
    for i in range(16):
        state[i % 4][i // 4] = data[i]
    return state

def state_to_bytes(state):
    result = bytearray(16)
    for i in range(16):
        result[i] = state[i % 4][i // 4]
    return bytes(result)

# Test with known AES-128 block cipher test vector
# From FIPS 197
key = bytes.fromhex('2b7e151628aed2a6abf7158809cf4f3c')
plaintext = bytes.fromhex('48656c6c6f2c204145532d4543422100')

print(f"Key: {key.hex()}")
print(f"Plaintext: {plaintext.hex()}")

# Simplified AES (without key expansion, just use key directly)
# For a proper test, we need full key expansion
# Let's just trace the transformations

state = bytes_to_state(plaintext)
print(f"\nInitial state (column-major):")
for i in range(4):
    print(f"  Column {i}: {state[0][i]:02x} {state[1][i]:02x} {state[2][i]:02x} {state[3][i]:02x}")

# Apply SubBytes
sub_bytes(state)
print(f"\nAfter SubBytes:")
for i in range(4):
    print(f"  Column {i}: {state[0][i]:02x} {state[1][i]:02x} {state[2][i]:02x} {state[3][i]:02x}")

# Apply ShiftRows
shift_rows(state)
print(f"\nAfter ShiftRows:")
for i in range(4):
    print(f"  Column {i}: {state[0][i]:02x} {state[1][i]:02x} {state[2][i]:02x} {state[3][i]:02x}")

# Apply MixColumns
mix_columns(state)
print(f"\nAfter MixColumns:")
for i in range(4):
    print(f"  Column {i}: {state[0][i]:02x} {state[1][i]:02x} {state[2][i]:02x} {state[3][i]:02x}")
