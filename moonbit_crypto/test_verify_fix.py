...
def test_shift_rows():
    print("测试 ShiftRows 修复...")
    plaintext = b"Hello, AES-ECB!X"  # 16 bytes
    ...
    print(f"测试结果: {'PASS' if result == plaintext else 'FAIL'}")

def test_sub_mix():
    print("\n测试 SubBytes + MixColumns...")
    plaintext = b"Hello, AES-ECB!X"  # 16 bytes
    state = bytes_to_state(plaintext)
    ...
    result = state_to_bytes(state)
    print(f"Mixed 结果前16字节: {result.hex()}")
    print("需要通过解密验证")
